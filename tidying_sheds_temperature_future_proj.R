...
save(sheds_temp_metrics_huc_join, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc_join_pls4.RData")
...
load(file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc_join_pls4.RData")

metrics12_pls4 <- sheds_temp_metrics_huc_join %>% 
  group_by(huc12_name) %>% 
  summarise(max_temp = mean(max_temp, na.rm = T),
            mean_jul_temp = mean(mean_jul_temp, na.rm = T),
            mean_summer_temp = mean(mean_summer_temp, na.rm = T),
            mean_max_temp_30d = mean(max_temp_30d, na.rm = T),
            mean_n_day_gt_22 = mean(n_day_temp_gt_22, na.rm = T))
save(metrics12_pls4, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc12_pls4.RData")



metrics10_pls4 <- sheds_temp_metrics_huc_join %>% 
  group_by(huc10_name) %>% 
  summarise(max_temp = mean(max_temp, na.rm = T),
            mean_jul_temp = mean(mean_jul_temp, na.rm = T),
            mean_summer_temp = mean(mean_summer_temp, na.rm = T),
            mean_max_temp_30d = mean(max_temp_30d, na.rm = T),
            mean_n_day_gt_22 = mean(n_day_temp_gt_22, na.rm = T))
save(metrics10_pls4, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc10_pls4.RData")



metrics08_pls4 <- sheds_temp_metrics_huc_join %>% 
  group_by(huc8_name) %>% 
  summarise(max_temp = mean(max_temp, na.rm = T),
            mean_jul_temp = mean(mean_jul_temp, na.rm = T),
            mean_summer_temp = mean(mean_summer_temp, na.rm = T),
            mean_max_temp_30d = mean(max_temp_30d, na.rm = T),
            mean_n_day_gt_22 = mean(n_day_temp_gt_22, na.rm = T))
save(metrics08_pls4, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc08_pls4.RData")



#plot temperatures

load(file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/sheds_temp_metrics_huc12_pls4.RData")

dat <- left_join(huc12, metrics12_pls4, by = "huc12_name")
...
ggplot()+
  ...
  ggtitle(label = "mean summer temperature plus 4")+
