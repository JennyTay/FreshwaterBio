...
pred <- pred %>%
  data.frame %>% 
  pivot_longer(40:92, values_to = "propabun", names_to = "common_name") 
...
d <- ggplot(data = FA)+
  ...
  facet_wrap(~tolerance)+
    theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 15))+
  ggtitle("Proportional Abundance: Habitat Guilds")+
  labs(fill = "PA")
...
df8 <- df7 %>% 
  filter(trait %in% c("TC_1", "TC_2", "TC_3", "TC_4", "TC_5", "TC_6", 
                      "FC_1", "FC_2", "FC_3", "FC_4", "FC_5", "FC_6","native", "introduced" ))
df9 <- df7 %>% 
