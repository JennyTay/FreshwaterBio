...
ggplot(data = NHDplusV2_NewEngCrop_baseline)+
  geom_sf(aes(color = propabun_eastern.blacknose.dace.rds))+
...
pred <- pred %>%
  data.frame %>% 
  pivot_longer(40:96, values_to = "propabun", names_to = "common_name") 
...
#plot cold water native species proportional abundance
coldnat_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "cold") %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(coldnatpropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(coldnatpropabund = mean(coldnatpropabund, na.rm = T))
coldnat_1 <- left_join(huc12, coldnat_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
coldnat_1$coldnatpropabund <- ifelse(coldnat_1$coldnatpropabund > 1, 1, coldnat_1$coldnatpropabund)

cldnprob <- ggplot(data = coldnat_1)+
  geom_sf(aes(fill = coldnatpropabund), color = NA)+
  ...
  ggtitle("proportional abundance")+
    geom_text(label = "Cold",x=-72.5,y=47, color = "black", size = 5)+
  labs(fill = NULL)

#plot cool water native species proportional abundance
coolnat_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "cool") %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(coolnatpropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(coolnatpropabund = mean(coolnatpropabund, na.rm = T))
coolnat_1 <- left_join(huc12, coolnat_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
coolnat_1$coolnatpropabund <- ifelse(coolnat_1$coolnatpropabund > 1, 1, coolnat_1$coolnatpropabund)

coolnprob <- ggplot(data = coolnat_1)+
  geom_sf(aes(fill = coolnatpropabund), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 0.5,
...
#plot warm water native species proportional abundance
warmnat_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "warm") %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(warmnatpropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(warmnatpropabund = mean(warmnatpropabund, na.rm = T))
warmnat_1 <- left_join(huc12, warmnat_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
warmnat_1$warmnatpropabund <- ifelse(warmnat_1$warmnatpropabund > 1, 1, warmnat_1$warmnatpropabund)

wnprob <- ggplot(data = warmnat_1)+
  geom_sf(aes(fill = warmnatpropabund), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 0.5,
...
#plot warm water nonnative species proportional abundance
# warmnonnat_1 <- pred %>% 
#   select(COMID, huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
#   unique() %>% 
#   filter(tmp == "warm" & origin == "introduced")  %>%
#   group_by(COMID, huc12_name, huc12_tnmid) %>% 
#   summarise(warmnonnatpropabund = sum(propabun, na.rm = T)) %>% 
#   group_by(huc12_name, huc12_tnmid) %>% 
#   summarise(warmnonnatpropabund = mean(warmnonnatpropabund))
# warmnonnat_1 <- left_join(huc12, warmnonnat_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
# wnnprob <- ggplot(data = warmnonnat_1)+
#   geom_sf(aes(fill = warmnonnatpropabund), color = NA)+
#   scale_fill_gradient2(low = "grey60",
#                         mid = "yellow",
#                         high = "green3",
#                         midpoint = 0.7,
...
#plot fluvial specialist species proportional abundance
specialist_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, tolerance, propabun) %>% 
  unique() %>% 
  filter(tolerance == "fluvial specialist") %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(spenatpropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(spenatpropabund = mean(spenatpropabund, na.rm = T))
specialist_1 <- left_join(huc12, specialist_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
specialist_1$spenatpropabund <- ifelse(specialist_1$spenatpropabund > 1, 1, specialist_1$spenatpropabund)

specprob_1 <- ggplot(data = specialist_1)+
  geom_sf(aes(fill = spenatpropabund), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .5,
...
dependant_1 <- pred %>% 
  ...
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(deppropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(deppropabund = mean(deppropabund, na.rm = T))
dependant_1 <- left_join(huc12, dependant_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
dependant_1$deppropabund <- ifelse(dependant_1$deppropabund > 1, 1, dependant_1$deppropabund)

depprop_1 <- ggplot(data = dependant_1)+
  geom_sf(aes(fill = deppropabund), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 0.5,
...
generalist_1 <- pred %>% 
  ...
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(genpropabund = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(genpropabund = mean(genpropabund, na.rm = T))
generalist_1 <- left_join(huc12, generalist_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
generalist_1$genpropabund <- ifelse(generalist_1$genpropabund > 1, 1, generalist_1$genpropabund)

genprob_1 <- ggplot(data = generalist_1)+
  geom_sf(aes(fill = genpropabund), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 0.5,
...
geog_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, geogclust, propabun) %>% 
  unique() %>% 
  filter(geogclust == 1)  %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(geog_1 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(geog_1 = mean(geog_1))
geog_1 <- left_join(huc12, geog_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
geog_1prob <- ggplot(data = geog_1)+
  geom_sf(aes(fill = geog_1), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 0.08,
...
geog_2 <- pred %>% 
  ...
  filter(geogclust == 2)  %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(geog_2 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(geog_2 = mean(geog_2))
geog_2 <- left_join(huc12, geog_2, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
geog_2prob <- ggplot(data = geog_2)+
  geom_sf(aes(fill = geog_2), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = 1.4,
...
geog_3 <- pred %>% 
  ...
  filter(geogclust == 3)  %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(geog_3 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(geog_3 = mean(geog_3))
geog_3 <- left_join(huc12, geog_3, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
geog_3prob <- ggplot(data = geog_3)+
  geom_sf(aes(fill = geog_3), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .75,
...
clim_1 <- pred %>% 
  select(COMID, huc12_name, huc12_tnmid, common_name, climclust, propabun) %>% 
  unique() %>% 
  filter(climclust == 1)  %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(clim_1 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(clim_1 = mean(clim_1))
clim_1 <- left_join(huc12, clim_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))


clim_1prob <- ggplot(data = clim_1)+
  geom_sf(aes(fill = clim_1), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .5,
...
clim_2 <- pred %>% 
  ...
  filter(climclust == 2) %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(clim_2 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(clim_2 = mean(clim_2))
clim_2 <- left_join(huc12, clim_2, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
clim_2prob <- ggplot(data = clim_2)+
  geom_sf(aes(fill = clim_2), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .5,
...
clim_3 <- pred %>% 
  ...
  filter(climclust == 3) %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(clim_3 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(clim_3 = mean(clim_3))
clim_3 <- left_join(huc12, clim_3, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
clim_3prob <- ggplot(data = clim_3)+
  geom_sf(aes(fill = clim_3), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .5,
...
clim_4 <- pred %>% 
  ...
  filter(climclust == 4) %>%
  group_by(COMID, huc12_name, huc12_tnmid) %>% 
  summarise(clim_4 = sum(propabun, na.rm = T)) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(clim_4 = mean(clim_4))
clim_4 <- left_join(huc12, clim_4, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
clim_4$clim_4 <- ifelse(clim_4$clim_4 > 1, 1, clim_4$clim_4)

clim_4prob <- ggplot(data = clim_4)+
  geom_sf(aes(fill = clim_4), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .5,
...
#plot brook trout propabun
brktrt_1 <- pred %>% 
  select(huc12_name, huc12_tnmid, common_name, propabun) %>% 
  unique() %>% 
  filter(common_name == "brook trout",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(brktrt_propabun = mean(propabun, na.rm = T))
brktrt_1 <- left_join(huc12, brktrt_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
brkprop <- ggplot(data = brktrt_1)+
  geom_sf(aes(fill = brktrt_propabun), color = NA)+
  scale_fill_gradient2(low = "grey60",
                        mid = "yellow",
                        high = "green3",
                        midpoint = .35,
...
pmpk_1 <- pred %>% 
  ...
  filter(common_name == "pumpkinseed",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(pmpk_propabun = mean(propabun, na.rm = T))
pmpk_1 <- left_join(huc12, pmpk_1, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
pmpprop <- ggplot(data = pmpk_1)+
  geom_sf(aes(fill = pmpk_propabun, color = pmpk_propabun))
...
pmpk <- pred_zero %>% 
  ...
  filter(common_name == "pumpkinseed",
         probZero < .9) %>% 
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(pmpkprob = 1 - (mean(probZero)))
pmpk <- left_join(huc12, pmpk, by = c("name" = "huc12_name", "tnmid" = "huc12_tnmid"))
pmprich <- ggplot(data = pmpk)+
  geom_sf(aes(fill = pmpkprob, color = pmpkprob))
...
cc <- arrangeGrob(specprob_1, specialist_rich1, 
                 depprop_1, dependent_2rich, 
                 genprob_1, generalist_rich3,
             ncol=2, nrow =3)
ggsave(cc, file = "tmpfigures/paper1/BaselineFishToleranceDist.png", 
    width = 12,
    height = 15,
    units = "cm",
    dpi = 300)
...
d <- arrangeGrob(clim_1prob, clim_rich1, 
                 clim_2prob, clim_rich2, 
                 clim_3prob, clim_rich3,
                 clim_4prob, clim_rich4,
             ncol=2, nrow =4)
...
clust <- bystate %>% 
  ...
  summarise(clust_avg_prob = round(mean(total_prob, na.rm = T),2), #average prob zero within each COMID across the state
            clust_total_prob = round(sum(total_prob, na.rm = T), 0))  #total prob zero within each COMID across the state


ggplot(data = temp, mapping = aes(y = state, x = tmp_avg_prob, fill = tmp))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  geom_text(aes(label = tmp_avg_prob), position=position_dodge(width=0.9),hjust = 1.2)

ggplot(data = temp, mapping = aes(y = state, x = tmp_total_prob, fill = tmp))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  geom_text(aes(label = tmp_total_prob), position=position_dodge(width=0.9),hjust = 1.2)

ggplot(data = tol, mapping = aes(y = state, x = tol_avg_prob, fill = tolerance))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  geom_text(aes(label = tol_avg_prob), position=position_dodge(width=0.9),hjust = 1.2)

ggplot(data = tol, mapping = aes(y = state, x = tol_total_prob, fill = tolerance))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  geom_text(aes(label = tol_total_prob), position=position_dodge(width=0.9),hjust = 1.2)

ggplot(data = clust, mapping = aes(y = state, x = clust_total_prob, fill = as.factor(climclust)))+
  geom_bar(stat = "identity", position = "dodge", color = "black")+
  geom_text(aes(label = clust_total_prob), position=position_dodge(width=0.9),hjust = -.1)


test <- bystate %>% 
  data.frame() %>% 
  filter(!is.na(tmp)) %>% 
  select(COMID, huc8_name, tmp, ZeroProb) %>% 
  mutate(probOcc = 1 - ZeroProb) %>% 
  unique() %>% 
  group_by(COMID, huc8_name, tmp) %>% 
  summarise(total_prob = sum(probOcc, na.rm = T)) %>% 
  ungroup %>% 
  group_by(huc8_name, tmp) %>% 
  summarise(avg_prob = round(mean(total_prob, na.rm = T),2), #average prob zero within each COMID across the state
            total_prob = round(sum(total_prob, na.rm = T), 0))  #total prob zero within each COMID across the state

ggplot(data = test, mapping = aes(y = huc8_name, x = avg_prob, fill = tmp))+
  geom_bar(stat = "identity", position = "dodge", color = "black")

ggplot(data = test, mapping = aes(y = huc8_name, x = total_prob, fill = tmp))+
  geom_bar(stat = "identity", position = "dodge", color = "black")
