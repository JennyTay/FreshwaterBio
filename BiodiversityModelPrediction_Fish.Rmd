...
aa <- ggplot(data = OR)+
  ...
  ggtitle("A) Native to Introduced")+
  labs(fill = "Ratio")
...
US <- US %>% 
  filter(!STUSPS %in% c("HI", "GU", "AK", "VI", "MP", "PR", "AS"))
...
locationalMap <- ggplot(data = US)+
  ...
  geom_sf(data = states, fill = "red2")+
    theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        title = element_text(size = 10))+
  ggtitle("D) Study Location")




fig2 <- gridExtra::grid.arrange(c,e,d,locationalMap, nrow = 2, ncol = 2, widths = c(3/4 , 1/4))
ggsave(fig2, file = "tmpfigures/paper1/fig2.png")
...
df8$trait[df8$trait == "native"] <- "Native"
df8$trait[df8$trait == "introduced"] <- "Introduced"
df9 <- df7 %>% 
