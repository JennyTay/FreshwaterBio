...
coldnat_1 <- pred %>% 
  select(huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "cold" & origin == "native",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(coldnatpropabund = mean(propabun, na.rm = T))
...
cldnprob <- ggplot(data = coldnat_1)+
  ...
  scale_fill_gradient2(low = "grey60",
    ...
    geom_text(label = "Cold Native",x=-72.5,y=47, color = "black", size = 5)+
  labs(fill = "Prop abun")
...
coolnat_1 <- pred %>% 
  select(huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "cool" & origin == "native",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(coolnatpropabund = mean(propabun, na.rm = T))
...
coolnprob <- ggplot(data = coolnat_1)+
  ...
  scale_fill_gradient2(low = "grey60",
    ...
    theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 15))+
    geom_text(label = "Cool Native",x=-72.5,y=47, color = "black", size = 5)+
  labs(fill = NULL)
...
warmnat_1 <- pred %>% 
  select(huc12_name, huc12_tnmid, common_name, tmp, origin, propabun) %>% 
  unique() %>% 
  filter(tmp == "warm" & origin == "native",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(warmnatpropabund = mean(propabun, na.rm = T))
...
wnprob <- ggplot(data = warmnat_1)+
  ...
  scale_fill_gradient2(low = "grey60",
    ...
    theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 15))+
    geom_text(label = "Warm Native",x=-72.5,y=47, color = "black", size = 5)+
  labs(fill = NULL)
...
warmnonnat_1 <- pred %>% 
  ...
  filter(tmp == "warm" & origin == "introduced",
         propabun > .1) %>%
  group_by(huc12_name, huc12_tnmid) %>% 
  summarise(warmnonnatpropabund = mean(propabun, na.rm = T))
...
wnnprob <- ggplot(data = warmnonnat_1)+
  ...
  scale_fill_gradient2(low = "grey60",
    ...
    theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 15))+
    geom_text(label = "Warm Non-native",x=-72.5,y=47, color = "black", size = 5)+
  labs(fill = NULL)

#plot native species shannon index
...
coldnat <- pred_zero %>% 
  select(huc12_name, huc12_tnmid, common_name, tmp, origin, probZero) %>% 
  unique() %>% 
  filter(tmp == "cold" & origin == "native",
         probZero < .9) %>%
...
c <- arrangeGrob(cldnprob, cldnrich, 
                 ...
                 brkprop, brkrich,
             ncol=2, nrow =5)
ggsave(c, file = "tmpfigures/testing123.png", 
    width = 12,
    height = 20,
    units = "cm",
    dpi = 300)
...
#plot each proportional abundance variable versus its richness counterpart
...
compare <- data.frame(
  ...
  brktrt_propabun = brktrt_1$brktrt,
