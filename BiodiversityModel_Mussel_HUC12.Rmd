...
In the third part of the script we model species richness and shannon index by HUC12
...
colnames(predictions) <- gsub("_Mgmt_Paper_propabun_", "pres_", colnames(predictions)) #the _Mgmt_Paper_propabun_ is the title of the model, but we used the zero predict option so we need to change the column names
...
#save host fish files for Becca O
# save(fish_host_occ, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/fish_host_occ.RData")
# save(fish_host_prob_occ, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/fish_host_prob_occ.RData")
# save(fish_host_propabun, file = "C:/Users/jenrogers/Documents/necascFreshwaterBio/model_datafiles/fish_host_propabun.RData")



#plot fish host occurrence for fish partner review
...
ggplot(data = plot)+
    geom_sf(aes(fill = as.character(YLamp_notmodeledhost)), color = NA)+
  geom_sf(data = huc8, fill = NA, color = "grey70", lwd = 0.15)+
  geom_sf(data = pres[pres$common_name== "yellow lampmussel", ], 
            color = "black", size = .6)+
    theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          panel.background = element_blank())+
  labs(fill = "YLamp_notmodeledhost")

ggsave(paste("tmpfigures/YLamp_notmodeledhost.png"), 
    plot = last_plot(),
    width = 18,
    height = 12,
    units = "cm",
...
#join mussel_occurrence data to the mussel event huc join so that we can get presence and absence by huc12
...
#### Removed Afloa_notmodeledhost from the model becuase of the huge gaps in low ct river and coastal maine.
dat_AF <- dat %>% 
...
# make composite OR plots to show how different clusters and trait groups respond to the variables on average

mussel_results_with_host.csv <- read.csv("tmpfigures/mussel_results_with_host.csv")
#prepare the zero inflation results file
results_zer <- mussel_results_with_host.csv %>% 
  arrange(common_name) %>% 
  filter(variable != "(Intercept)") %>% 
  select(common_name, variable, odds, up_odds, lw_odds, p_val)
...
#composite OR figures by cluster 
clusters <- read.csv("tmpfigures/musselclusters_byStreamflowAndTemp.csv")
clusters <- clusters %>% 
  select(-1) %>% 
...
composit_OR_figs$variable[composit_OR_figs$variable == "annual_mean_summer_temp"] <- "Mean summer temp"
composit_OR_figs$variable[composit_OR_figs$variable == "BFI_HIST"] <- "Baseflow index (rch)"
...
composit_OR_figs$variable[composit_OR_figs$variable == "pctWetland_Cat"] <- "Wetland (cat)"
composit_OR_figs$variable[composit_OR_figs$variable == "PctImp_WsRp100"] <- "Impervious (Rip)"
composit_OR_figs$variable[composit_OR_figs$variable == "pctNatural_ripbuf100"] <- "Natural (Rip)"
composit_OR_figs$variable[composit_OR_figs$variable == "pctForest_ws"] <- "Forest (Ws)"
composit_OR_figs$variable[composit_OR_figs$variable == "pctAg_Ws"] <- "Agriculture (Ws)"
composit_OR_figs$variable[composit_OR_figs$variable == "huc12_damden_sqkm"] <- "Dam density"
composit_OR_figs$variable[composit_OR_figs$variable == "KffactWs"] <- "Soil erodibility"
composit_OR_figs$variable[composit_OR_figs$variable == "mean_prob_occ"] <- "All fish"
composit_OR_figs$variable[composit_OR_figs$variable == "CaOWs"] <- "Calcium"


#remove host fish specialist variables because these are only in models for one species and wont represent avages over spp within each group
composit_OR_figs <- composit_OR_figs %>% 
  filter(!variable == "YLamp_perferredhost")


#order the variables so they occurr in the correct/ same order for all figures
composit_OR_figs$variable <- factor(composit_OR_figs$variable, levels = c("Mean summer temp", "Baseflow index (rch)", "Winter floods", 
                                                          "Low flow date", 
                                                          "Wetland (cat)", "Impervious (Rip)", "Natural (Rip)", "Forest (Ws)", "Agriculture (Ws)", 
                                                          "Dam density", 
                                                          "Soil erodibility",
                                                          "All fish",
                                                          "Calcium", "Latitude", "Longitude", "Elevation", "Watershed area"))

composit_OR_fig_Flowtmpclust <- composit_OR_figs%>% 
  filter(!is.na(cluster_TP), !is.na(variable)) %>%
  group_by(cluster_TP, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)


composit_OR_fig_status <- composit_OR_figs%>% 
  filter(!is.na(status), !is.na(variable)) %>%
  group_by(status, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_status$status <- str_to_title(composit_OR_fig_status$status)

composit_OR_fig_size <- composit_OR_figs%>% 
  filter(!is.na(size), !is.na(variable)) %>%
  group_by(size, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_size$size <- str_to_title(composit_OR_fig_size$size)

composit_OR_fig_host <- composit_OR_figs%>% 
  filter(!is.na(host), !is.na(variable)) %>%
  group_by(host, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_host$host <- str_to_title(composit_OR_fig_host$host)

composit_OR_fig_strat <- composit_OR_figs%>% 
  filter(!is.na(strat), !is.na(variable)) %>%
  filter(strat %in% c("equilibrium" , "opportunistic", "periodic")) %>%
  group_by(strat, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_strat$strat <- str_to_title(composit_OR_fig_strat$strat)

composit_OR_fig_rank <- composit_OR_figs%>% 
  filter(!is.na(rank), !is.na(variable)) %>%
  group_by(rank, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_rank$rank <- str_to_title(composit_OR_fig_rank$rank)

composit_OR_fig_shell <- composit_OR_figs%>% 
  filter(!is.na(shell), !is.na(variable)) %>%
  filter(shell %in% c("thin" , "thick")) %>% 
  group_by(shell, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_shell$shell <- str_to_title(composit_OR_fig_shell$shell)

composit_OR_fig_habitat <- composit_OR_figs%>% 
  filter(!is.na(habitat), !is.na(variable)) %>%
  group_by(habitat, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_habitat$habitat <- str_to_title(composit_OR_fig_habitat$habitat)

composit_OR_fig_brood <- composit_OR_figs%>% 
  filter(!is.na(brood), !is.na(variable)) %>%
  filter(brood %in% c("long" , "short")) %>%
  group_by(brood, variable) %>% 
  summarise(OR = median(odds, na.rm = T),
            ORCIlow = median(lw_odds, na.rm = T),
            ORCIhigh = median(up_odds, na.rm = T)) %>% 
  filter(ORCIlow  > 1 & ORCIhigh > 1 | ORCIlow  < 1 & ORCIhigh < 1)
composit_OR_fig_brood$brood <- str_to_title(composit_OR_fig_brood$brood)



#Manuscript Figures
a <- ggplot(data = composit_OR_fig_Flowtmpclust, aes(x = variable, y = OR))+
  ...
  facet_wrap(~as.factor(cluster_TP), nrow = 1)+
  ...
  theme(panel.border = element_rect(colour = "black", fill = NA),
        ...
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = "Odds ratio", title = "A) Stream Temperature and streamflow Clusters")

ggsave(a, file = "tmpfigures/paper2/CompositORPlots1.png", 
    width = 25,
    height = 15,
    units = "cm",
    dpi = 300)
...
b <- ggplot(data = composit_OR_fig_status, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(status), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.text = element_text(size = 10),
        ...
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "A) Status")

c <- ggplot(data = composit_OR_fig_rank, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(rank), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "B) Species Ranking")

d <- ggplot(data = composit_OR_fig_size, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(size), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "C) Size")

e <- ggplot(data = composit_OR_fig_shell, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(shell), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "D) Shell Thickness")

f <- ggplot(data = composit_OR_fig_host, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(host), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "A) Fish Host")

g <- ggplot(data = composit_OR_fig_strat, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(strat), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "B) Life History Strategy")

h <- ggplot(data = composit_OR_fig_brood, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(brood), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "C) Brooding Duration")


i <- ggplot(data = composit_OR_fig_habitat, aes(x = variable, y = OR))+
  geom_point(size = 1.5, position=position_dodge(width=0.5))+
  geom_errorbar(aes(ymin = ORCIlow, ymax = ORCIhigh), position=position_dodge(width=0.5))+
  coord_flip(ylim = c(0,3))+
  facet_wrap(~as.factor(habitat), nrow = 1)+
  geom_hline(yintercept = 1, color  = "red", linetype = "dashed", size = 1)+
  theme(panel.border = element_rect(colour = "black", fill = NA),
          panel.background = element_blank(),
        axis.title = element_text(size = 15),
        axis.title.y = element_blank(),
        title = element_text(size = 15))+
  labs(y = NULL, title = "D) Habitat Specificity")


library(cowplot)
j <- plot_grid(b,c,d,e, align = "h", nrow = 2)
ggsave(j, file = "tmpfigures/paper2/CompositORPlots2.png", 
    width = 25,
    height = 15,
    units = "cm",
    dpi = 300)

k <- plot_grid(f,g,h,i, align = "h", nrow = 2)
ggsave(k, file = "tmpfigures/paper2/CompositORPlots2a.png", 
    width = 25,
    height = 15,
    units = "cm",
    dpi = 300)
