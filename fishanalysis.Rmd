...
logistreg <- left_join(fish_occurrence, fish_event_flowline_join, by = "UID") %>% 
  select(common_name, occurrence, StreamOrde, TotalDASqK, Slope, geometry)
...
#add column for log(TotalDASqK) and log(Slope) because these are highly skewed
logistreg$log_Slope <- log(logistreg$Slope)
logistreg$log_TotalDASqK <- log(logistreg$TotalDASqK)
...
library(corrplot)
correlation <- cor(logistreg[, c(3, 7, 8, 9, 10)])
...
Chunk8
2. First prepare the fish count data (like we did in chunk 4 for the presence absence data) 
