...
absence$common_name <- rep(fish, times = 23107)
...
#filter the common names, the UIDs, and keep only the data from the first run.
...
fish_count <- left_join(fish_count, method, by = "UID")

#hmmmm do we want to add in the zero data???????
#add on the zero data 
#first filter zeros from the fish_occurrence ddata, and then add the columns to align with the fish count data
zero <- fish_occurrence %>% 
  filter(occurrence == 0) %>% 
  select(-occurrence) %>% 
  mutate(run_num = 1,
         count = 0)

zero <- left_join(zero, method, by = "UID")

fish_count_with_zeros <- rbind(fish_count, zero)

 


 
ggplot(data = fish_count, aes(x = count))+
  geom_histogram()+
  xlim(1,100)
...
countmdl <- left_join(fish_count, fish_event_flowline_join, by = "UID") %>% 
...
join the fish length data to the fish_event_flowline_join, and run a linear regression using lat, long, total upstream drainage area, slope. In this model, we can use all of the data, regardless of reach lenght and total pick up and gear.
...
#remove the blacknose dace observatin that is greater than 7000mm - clearly an error
fish_size <- fish_size %>% 
  filter(length_mm <7000)
...
#linearly regress fish length verse elevation, stream order, basin area, slope, lat and long
#left off here!
