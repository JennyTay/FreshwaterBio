...
This is the BiodiversityModelPrediction_Mussel_HUC12 script, but updated to use the fish model results fro the BiodiversityModel_ClimateChangePaper script - ie its been agjusted to include riparian imperiousness(Ws), riparian forest(cat), and watershed forest % in the model, as opposed to watershed imperiousness.   This model will be used for the climate change and management paper (we are still using the original model in the paper 1)
...
#management scenario #1 - water quality scenario (decrease PctImp_WsRp100 and increase pctNatural_ripbuf100)
...
#management scenario #1 - water quality scenario (decrease PctImp_WsRp100 to 0, increase pctNatural_ripbuf100 by 100)
...
#management scenario #2 - connectivity scenario (decrease huc12_damden_sqkm to 0)
...
MANAGEMENT (watershed forest restoration) AND CLIMATE CHANGE SCENARIO
 Now, calculate mussel probability of occurrence with the climate change AND management scenario for 2080
...
#management scenario - Watershed forest scenario (increase pctForest_ws to 100%)
...
MANAGEMENT (catchment wetland) AND CLIMATE CHANGE SCENARIO
 Now, calculate mussel probability of occurrence with the climate change AND management scenario for 2080
 first, re-calculate the host fish covariates with the climate change data and improved wetlands
```{r}
...
#management scenario - catchment wetlands (increase pctWetland_Cat by 25 percent)

test$pctWetland_Cat <- test$pctWetland_Cat*1.25
...
#join the scaled variables back to 
NHDplusV2_NewEngCrop_wetland <- cbind((NHDplusV2_NewEngCrop_2080)[,c(1:6, 9)], test)
...
predictions_wetland <- data.frame("test" = (1 - (predict(mdl, newdata = NHDplusV2_NewEngCrop_wetland, type = "zprob")))) #do 1 - predict because we want prob of occurrence, not the prob of a zero
names(predictions_wetland)[1] <- file.list[i]
...
for (i in 2:53) { 
  ...
  tmp <- data.frame("test" = (1 - (predict(mdl, newdata = NHDplusV2_NewEngCrop_wetland, type = "zprob"))))
  names(tmp)[1] <- file.list[i]
  
  predictions_wetland <- cbind(predictions_wetland, tmp)
  
  
}

colnames(predictions_wetland) <- gsub("_Mgmt_Paper_propabun_", "pres_", colnames(predictions_wetland)) #the probabun is the title of the model, but we used the zero predict option so we need to change the column names


#join the predictions_wetland to the list of comids
tmp <- cbind(NHDplusV2_NewEngCrop_wetland, predictions_wetland)


#first average by HUC12 and by species and save cc fish predictions
fish_wetland_proboccurrence_huc12 <- tmp %>%
  data.frame() %>% 
  select(huc12_name, huc12_tnmid, 43:95) %>% 
  filter(!is.na(pres_brook.trout.rds),
         !is.na(huc12_name)) %>%
  ...
  summarise(mean_prob_occ = mean(prob_occ)) %>% 
  ungroup() %>% 
  mutate(scenario = "wetland", taxa = "fish")
save(fish_wetland_proboccurrence_huc12, file = 'ProjectResults/fish_wetland_proboccurrence.RData')


#average by HUC12 becuase that is the scale we are modeling mussels
fish_host_prob_occ_wetland <- tmp %>%
  data.frame() %>% 
  select(huc12_name, huc12_tnmid, COMID, 43:95) %>% 
  filter(!is.na(pres_brook.trout.rds),
         !is.na(huc12_name)) %>%
...
predictions_wetland <- data.frame("test" = predict(mdl, newdata = NHDplusV2_NewEngCrop_wetland, type = "response"))
names(predictions_wetland)[1] <- file.list[i]
...
for (i in 2:53) { 
  ...
  tmp <- data.frame("test" = predict(mdl, newdata = NHDplusV2_NewEngCrop_wetland, type = "response"))
  names(tmp)[1] <- file.list[i]
...
#select only the columns for preferred host fish
predictions_wetland <- predictions_wetland %>% 
  select("_Mgmt_Paper_propabun_brown trout.rds", "_Mgmt_Paper_propabun_rainbow trout.rds", "_Mgmt_Paper_propabun_atlantic salmon.rds", "_Mgmt_Paper_propabun_brook trout.rds",
         "_Mgmt_Paper_propabun_yellow perch.rds", "_Mgmt_Paper_propabun_white perch.rds", "_Mgmt_Paper_propabun_largemouth bass.rds", "_Mgmt_Paper_propabun_bluegill.rds", "_Mgmt_Paper_propabun_pumpkinseed.rds", 
         "_Mgmt_Paper_propabun_redbreast sunfish.rds", )
names(predictions_wetland) <- gsub("_Mgmt_Paper_propabun_", "", names(predictions_wetland))
names(predictions_wetland) <- gsub(".rds", "", names(predictions_wetland))
#names(predictions_wetland) <- gsub("\\.", " ", names(predictions_wetland))
...
#average by HUC10 becuase that is the scale we are modeling mussels
fish_host_propabun_wetland <- tmp %>%
  data.frame() %>% 
  select(COMID, huc12_name, huc12_tnmid, 43:52) %>% 
  filter(!is.na(brook.trout),
         !is.na(huc12_name)) %>%
...
apply the predictions to the climate change  projections for flow AND temperature combined with the management for wetlands
...
covariates_wetland <- left_join(NHDplusV2_NewEngCrop_mussel_covariates_HUC12, fish_host_prob_occ_wetland, by = c("huc12_name", "huc12_tnmid"))

covariates_wetland <- left_join(covariates_wetland, fish_host_propabun_wetland, by = c("huc12_name", "huc12_tnmid"))

covariates_wetland <- left_join(covariates_wetland, fish_host_occ, by = c("huc12_name", "huc12_tnmid"))
...
#remove the current flow and temperature values from the baseline file and add in the 2080 projections
order <- names(covariates_wetland)
covariates_wetland2 <- covariates_wetland %>% 
  select(-c(annual_mean_summer_temp, BFI_HIST, LO7Q1DT_HIST, CFM_HIST, W95_HIST, logMJJA_HIST)) %>% #remove the baseline values for these variables
  left_join(test, by = c("huc12_name", "huc12_tnmid")) %>%  #add in the projected 2080 values
  select(all_of(order))
...
covariates_wetland2$pctWetland_Cat <- covariates_wetland2$pctWetland_Cat * 1.25
...
#scale the variables using the means and sd used to scale the variables in the baseline condition
test <- covariates_wetland2[,c(3:42)]
test <- sweep(test, 2, means, FUN = '-') 
test <- sweep(test, 2, sd, FUN = '/')  

#join the scaled variables back to the full dataset
covariates_wetland <- cbind((covariates_wetland2)[,c(1:2)], test)
...
predictions <- data.frame("test" = predict(mdl, newdata = covariates_wetland, type = "response"))
names(predictions)[1] <- file.list[i]
...
for (i in 2:12) { 
  ...
  tmp <- data.frame("test" = predict(mdl, newdata = covariates_wetland, type = "response"))
  names(tmp)[1] <- file.list[i]
...
#join the predictions to the list of comids
prediction_wetland <- cbind(covariates_wetland, predictions)
...
prediction_wetland <- left_join(huc12, prediction_wetland, by = c("huc12_name", "huc12_tnmid"))


#save data files with mussel predictions for climate change/landcover mgmt
save(prediction_wetland, file = 'ProjectResults/mussel_CC2080_wetland_proboccurrence.RData')
...
ggsave(paste("tmpfigures/baseline_mussel_predictions/", names(plots)[i], "_wetland.png", sep = ""), 
    plot = last_plot(),
    width = 18,
    height = 12,
    units = "cm",
