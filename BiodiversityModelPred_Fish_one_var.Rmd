...
tmp_ccdvlp <- NHDplusV2_NewEngCrop_ccdvlp %>% 
  ...
  select(huc12_tnmid, huc12_name, 40:96) %>% 
  pivot_longer(3:59, names_to = "common_name") 
tmp_ccdvlp$common_name <- gsub(".rds", "", tmp_ccdvlp$common_name)
tmp_ccdvlp$common_name <- gsub("pres_", "", tmp_ccdvlp$common_name)
tmp_ccdvlp$common_name <- gsub("\\.", " ", tmp_ccdvlp$common_name)

tmp_ccdvlp <-  tmp_ccdvlp %>% 
  left_join(final_traits, by = "common_name") %>% 
  left_join(origin, by = "common_name")  %>% 
  left_join(temperature, by = "common_name") %>%  
  group_by(huc12_tnmid, huc12_name, origin, common_name) %>% 
  ...
  pivot_wider(names_from = origin, values_from = rich_ccdvlp) %>% 
  mutate(sum = sum(native, introduced, na.rm = T),
         prop_native_ccdvlp = native/sum) %>% 
  select(-introduced, -native, -'NA', -sum)
...
tmp_ccmgmt <- NHDplusV2_NewEngCrop_ccmgmt %>% 
  ...
  select(huc12_tnmid, huc12_name, 40:96) %>% 
  pivot_longer(3:59, names_to = "common_name") 
tmp_ccmgmt$common_name <- gsub(".rds", "", tmp_ccmgmt$common_name)
tmp_ccmgmt$common_name <- gsub("pres_", "", tmp_ccmgmt$common_name)
tmp_ccmgmt$common_name <- gsub("\\.", " ", tmp_ccmgmt$common_name)

tmp_ccmgmt <- tmp_ccmgmt %>% 
  left_join(final_traits, by = "common_name") %>% 
  left_join(origin, by = "common_name")  %>% 
  left_join(temperature, by = "common_name") %>%  
  group_by(huc12_tnmid, huc12_name, origin, common_name) %>% 
  ...
  pivot_wider(names_from = origin, values_from = rich_ccmgmt) %>% 
  mutate(sum = sum(native, introduced, na.rm = T),
         prop_native_ccmgmt = native/sum) %>% 
  select(-introduced, -native, -'NA', -sum)
...
huc12 <- left_join(huc12, tmp_ccdvlp, by = c("huc12_tnmid", "huc12_name"))
huc12 <- left_join(huc12, tmp_ccmgmt, by = c("huc12_tnmid", "huc12_name"))

#calcuate change in climate change from baseline, but change in the other two from climate change
huc12 <- huc12 %>% 
  mutate(change_ccdvlp = prop_native_ccdvlp - prop_native_cc,
         change_ccmgmt = prop_native_ccmgmt - prop_native_cc)
...
huc12a$scenario[huc12a$scenario == "prop_native_cc"] <- "Clim Change"
huc12a$scenario[huc12a$scenario == "prop_native_ccdvlp"] <- "Clim Chng + Develop"
huc12a$scenario[huc12a$scenario == "prop_native_ccmgmt"] <- "Clim Chng + Manage"
...
huc12b <- huc12 %>% 
  ...
  filter(!is.na(change))
huc12b$scenario[huc12b$scenario == "change_ccdvlp"] <- "Clim Chng + Develop"
huc12b$scenario[huc12b$scenario == "change_ccmgmt"] <- "Clim Chng + Manage"
...
tmp_ccdvlp <-  tmp_ccdvlp %>% 
  ...
  group_by(huc12_tnmid, huc12_name, tmp) %>% 
  summarise(rich_ccdvlp = n())%>% 
  pivot_wider(names_from = tmp, values_from = rich_ccdvlp) %>% 
  mutate(sum = sum(warm, cold, cool, na.rm = T),
         prop_cold_ccdvlp = cold/sum) %>% 
...
tmp_ccmgmt <- tmp_ccmgmt %>% 
  ...
  group_by(huc12_tnmid, huc12_name, tmp) %>% 
  summarise(rich_ccmgmt = n()) %>% 
  pivot_wider(names_from = tmp, values_from = rich_ccmgmt) %>% 
  mutate(sum = sum(warm, cold, cool, eury, na.rm = T),
         prop_cold_mgmt = cold/sum) %>% 
  select(-warm, -cold, -cool, -eury, -sum)
...
huc12 <- huc12 %>% 
  mutate(change_ccdvlp = prop_cold_ccdvlp - prop_cold_cc,
         change_ccmgmt = prop_cold_mgmt - prop_cold_cc)
...
huc12a$scenario[huc12a$scenario == "prop_cold_cc"] <- "Clim Change"
huc12a$scenario[huc12a$scenario == "prop_cold_ccdvlp"] <- "Clim Chng + Develop"
huc12a$scenario[huc12a$scenario == "prop_cold_ccmgmt"] <- "Clim Chng + Manage"
...
huc12a <- huc12 %>% 
  ...
  pivot_longer(2:3, names_to = "scenario", values_to = "change") %>% 
  filter(!is.na(change))
huc12a$scenario[huc12a$scenario == "change_ccdvlp"] <- "Clim Chng + Develop"
huc12a$scenario[huc12a$scenario == "change_ccmgmt"] <- "Clim Chng + Manage"
