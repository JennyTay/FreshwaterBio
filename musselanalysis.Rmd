...
Calcuate some metadata for the climate change paper (Nov 27, 2023)
```{r}
table(mussel_event$state)
dates <- mussel_event %>% 
  group_by(state) %>% 
  summarise(start = min(date, na.rm = T),
            end = max(date, na.rm = T))
min(mussel_event$date, na.rm = T)
max(mussel_event$date, na.rm = T)
nrow(mussel_event[mussel_event$date>mdy(01011985),]) / nrow(mussel_event)
```
...
nrow(mussel_event_huc_join) #total number of surveys - use this number as oppoed to the 4007 number
...
nrow(mussel_event_flowlineV2_join[mussel_event_flowlineV2_join$dist>500,])#1014 surveys are further than 500m
...
absence$common_name <- rep(mussel, times = 3971)
