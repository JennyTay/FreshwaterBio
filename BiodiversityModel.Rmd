...
counts <- fish_count_with_zeros %>%
  filter(gear == "efish_backpack",
         !is.na(reach_length_m),
         reach_length_m >0,
         !is.na(avg_reach_width_m),
         avg_reach_width_m >0,
         !is.na(efish_duration_s),
         efish_duration_s > 0) %>% 
  group_by(UID) %>% 
  ...
  mutate(abund_m_km_hr = count / (reach_length_m * avg_reach_width_m * efish_duration_s) *1000 * 3600) %>% 
...
dat <- dat %>% 
  select("UID", "COMID", "lat", "long",
         ...
         "count", "totalcount", "abund_m_km_hr", "common_name", 
...
dat3 <- dat2 %>% 
  select(count, abund_m_km_hr, lat, long, state, year, month, source, huc8_name,WsAreaSqKm, all_of(keep1), all_of(keep2), all_of(keep3), all_of(keep4)) 
...
dat3 <- dat3 %>% 
  ...
  select(occurrence, count, abund_m_km_hr, year, month, state, source, huc8_name, lat, long, 10:39)
...
model counts using all the variables
zero inflated negative binomial model
If count is 0 = its absent. If its >0, we model the count
```{r}
...
ggplot(data = dat3, mapping = aes(x = log(abund_m_km_hr)))+
  geom_histogram()

dat3$occurrence <- ifelse(dat3$count>0, 1, 0)
...
ggplot(data = compare_logreg, mapping = aes(x = occurrence, y = prob))+
  geom_point(alpha = 0.05, size = 5)


#model the log abundance

dat3$logabund <- log(dat3$abund_m_km_hr + 1)
...
compare_lm <- data.frame("abund" = dat3$logabund, "prob" = predict_lm)

ggplot(data = compare_lm, mapping = aes(x = abund, y = prob))+
  geom_point(alpha = 0.05, size = 5)+
  geom_smooth()

final <- data.frame("abundance" = dat3$logabund, predict_prob = compare_logreg$prob, predict_abund = compare_lm$prob)
final$prob_binary <- ifelse(final$predict_prob >0.2, 1, 0)
final$abundance_prediction <- final$prob_binary*final$predict_abund

ggplot(data = final, mapping = aes(x = abundance, y = abundance_prediction))+
  geom_point(alpha = 0.05, size = 5)+
  geom_smooth()
