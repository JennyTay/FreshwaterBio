...
temp_counts <- counts %>% 
  ...
  select(UID, tmp, propabun_tmp) %>% 
  pivot_wider(names_from = tmp, values_from = propabun_tmp) #LEFT OFF HERE!!! maybe need to make a ifelse statement, if cold >0.6, cold dominated...
...
temp_counts <- left_join(temp_counts, dat3, by = "UID")
