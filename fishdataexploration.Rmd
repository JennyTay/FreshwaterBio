...
#load fish data
shp <- st_read("C:/Users/jrogers/Documents/necascFreshwaterBio/SpatialData/sppdata/all_fish_count.shp")
#remove NA names
shp <- shp %>% 
...
dat2 <- dat %>% 
  filter(huc10name == "Ware River")%>% 
  group_by(common_name) %>% 
  summarise(count = sum(count))
write.csv(dat2, "tmpfigures/ware.csv")

dat3 <- dat %>% 
  filter(huc12name == "West Branch Ware River")%>% 
  group_by(common_name) %>% 
  summarise(count = sum(count))
write.csv(dat3, "tmpfigures/eastware.csv")
