...
ggsave(
    filename = "tmpfigures/surveybyyear.png",
    plot = last_plot(),
    width = 15,
    height = 10,
    units = "cm",
...
st_write(dat8, dsn = "nhd/huc8.shp")
st_write(dat10, dsn = "nhd/huc10.shp")
st_write(dat12, dsn = "nhd/huc12.shp")
st_write(flowline, dsn = "nhd/flowline.shp")
...
dat8 <- st_read("nhd/huc8.shp")
dat10 <- st_read("nhd/huc10.shp")
dat12 <- st_read("nhd/huc12.shp")
flowline <- st_read("nhd/flowline.shp")
...
dat <- fish_count %>% 
  ...
  mutate(huc10richness = n_distinct(common_name)) %>% 
  ungroup() %>% 
  group_by(year, huc12name) %>% 
  mutate(huc12richness = n_distinct(common_name)) %>% 
  ungroup() %>% 
  group_by(year, huc8name) %>% 
  mutate(huc8richness = n_distinct(common_name)) %>% 
  ungroup() %>% 
