...
#### Overview: 
This R script creates the biplots in Figure 2 of the manuscipt.  It uses the weighted mean of each environmental variable, weighted by the proportional abundance of that species at a survey site, such that ultimately, each of the 53 fish species are assigned a single value for each variable (i.e. mean annual stream temperature, percent of imperiousness within the upstream watershed, etc.).  We then used ordination to plot the similarity of each species in a 2-D figure, and we colored each species' point by the cluster. We also used arrows to show the direction of the relationship with the environmental variables.
