...
dat <- fish_event_huc_join %>% 
  ...
  mutate(count = ifelse(surveynum %in% c(1:5), "1-5",
                        ifelse(surveynum %in% c(6:25), "6-25",
                               ifelse(surveynum %in% c(26:100), "26-100", ">100"))))

dat$count <- factor(dat$count, levels = c("1-5", "6-25", "26-100", ">100"))
...
dat <- left_join(huc12, dat, by = c("name" = "huc12_name"))
 ...
 ggsave(
    filename = "C:/Users/jrogers/Documents/git/FreshwaterBio/FreshwaterBio/tmpfigures/surveynum_huc12.png",
    plot = last_plot(),
    width = 14,
    height = 9,
    units = "cm",
...
dat <- fish_event_huc_join %>% 
  ...
  group_by(huc10_name) %>% 
  summarise(surveynum = length(unique(UID))) %>% 
  mutate(count = ifelse(surveynum %in% c(1:5), "1-25",
                        ifelse(surveynum %in% c(6:25), "26-100",
                               ifelse(surveynum %in% c(26:100), "101-200", ">200"))))

dat$count <- factor(dat$count, levels = c("1-25", "26-100", "101-200", ">200"))
...
dat <- left_join(huc10, dat, by = c("Name" = "huc10_name"))
 ...
 ggsave(
    filename = "C:/Users/jrogers/Documents/git/FreshwaterBio/FreshwaterBio/tmpfigures/surveynum_huc10.png",
    plot = last_plot(),
    width = 14,
    height = 9,
    units = "cm",
...
dat <- fish_event_huc_join %>% 
  ...
  group_by(huc8_name) %>% 
  summarise(surveynum = length(unique(UID))) %>% 
  mutate(count = ifelse(surveynum %in% c(1:100), "1-100",
                        ifelse(surveynum %in% c(101:500), "101:500",
                               ifelse(surveynum %in% c(501:1000), "501:1000", ">1000"))))

dat$count <- factor(dat$count, levels = c("1-100", "101:500", "501:1000", ">1000"))
...
dat <- left_join(huc8, dat, by = c("name" = "huc8_name"))
 ...
 ggsave(
    filename = "C:/Users/jrogers/Documents/git/FreshwaterBio/FreshwaterBio/tmpfigures/surveynum_huc8.png",
    plot = last_plot(),
    width = 14,
    height = 9,
    units = "cm",
...
for( i in 1:length(unique(pres$common_name))){
  ...
  ggplot()+
    ...
    geom_sf(data = states, fill = NA, lwd = 0.25)+
...
#cluster based on the occurrence of other species for each HUC10, lumping all stream orders within each HUC 8
#use the proportional abundance instead of presence
...
counts <- fish_count_with_zeros %>%
  ...
  select(UID, propabun, common_name)
  
tmp <- left_join(counts, fish_event_huc_join, by = "UID") %>% 
  unique()
...
for (i in unique(tmp$huc10_name)) {
...
png(height = 10, width = 15, file = paste("tmpfigures/fish_clusters_huc10/", i, ".png", sep = ""), units = "in", res = 150, type = "cairo")

plot(hclust_avg, main = paste(i, "n = ", ncol(test), sep = " "), cex = 1.5)
...
test <- left_join(counts, dat3, by = "UID") %>% 
  mutate(occurrence = ifelse(count>0, 1, 0)) %>% 
  select(1, 4, 15,16, 22:47, 48) %>% 
  group_by(UID, lat, long, low_dur_days, high_dur_days, min_month, monthly_min_flow, max_month, annual_mean_max_temp_30d, BFI_HIST, LO7Q1DT_HIST, CFM_HIST, W95_HIST, BFIWs, ElevWs, WtDepWs, PctOw_Ws, PctUrbOp_Ws, PctBl_Ws, PctShrb_Ws, PctGrs_Ws, PctHay_Ws, PctWdWet_Ws, PctHbWet_Ws, PctImp_Ws, PctImp_WsRp100, pctforest_Ws, logWsAreaSqKm, logMJJA_HIST, common_name) %>%
  summarise(n = n()) %>%
  filter(n > 1)
unique(test$UID)
...
com <-  test[1:2000,25:60]
env <-  test[1:2000,2:24]
